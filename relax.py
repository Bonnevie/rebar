...
def RELAX(loss, control, conditional_control, logp,
    ...
    with tf.name_scope("RELAX"):
        ...
        hard_params_grads = []
        var_params_grads = [(tf.zeros_like(var_param), var_param) for var_param in var_params]
        ...
        with tf.name_scope("collect_grads"):
            for pure_grad, relax_grad, hard_param, score in gradcollection:
                ...
                with tf.name_scope("variance_grad"):
                    ...
                    var_params_grads = [(var_grad + 2*tf.reduce_sum(full_grad)*(grad_grad), param) for grad_grad, (var_grad, param) in zip(grad_grads, var_params_grads)]
...
if __name__ is "__main__":
    ...
    for categorical in [False, True]:
        ...
        grad, var_grad = RELAX(*rep.rebar_params(f, weight=nu), [Z], var_params=[temp_var, nu_var])
        ...
        gradc, varc_grad = RELAX(*repc.rebar_params(f, weight=nuc), [Z], var_params=[tempc_var, nuc_var])
